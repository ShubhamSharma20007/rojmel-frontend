...
import { createLedger, deleteLedgerById, getLedgerById, updateLedgerById } from '@/helper/api-communication';
    ...
    (async () => {
      ...
      if (request.statusCode === 200) {
        ...
        setFormData({
          ...
          cheque_pfms_clearing_date:request.data.cheque_pfms_clearing_date || ''
        })
      } else {
    ...
    if (validateForm()) {
      try {
        ...
        Toast.show({
          type: "error",
          text1: "❌ Error",
          text2: err.response?.data?.message || "An error occurred" || err?.message,
          text2Style: {
            fontSize: 12,
          },
        });
          ...
          <View style={styles.secondaryContainer}>
            ...
            >
              <Text style={[styles.dateText]}>
                {formData.transaction_date ? new Date(formData.transaction_date).toLocaleDateString() : 'Select Transaction Date'}
              </Text>
            </TouchableOpacity>
            {showDatePicker && (
              <DateTimePicker
                value={formData.transaction_date ? new Date(formData.transaction_date) as Date : new Date()}
                style={{
                  display: 'flex',
          ...
          {showClearingDatePicker && (
            ...
            <View style={{
              ...
              <View style={{
                ...
                <CustomButton text='Delete' handlePress={deletePayment} bg="red" />
              </View>
              <View style={{
                flex: 1
              }}>
